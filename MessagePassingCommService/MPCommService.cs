...
namespace MessagePassingComm
{
  ...
  public class Receiver : IMessagePassingComm
  {
    ...
    public bool openFileForWrite(string name, string toPath)
    {
      try
      {
        string writePath = Path.Combine(toPath, name);
  ...
  public class Sender
  {
    ...
    public bool postFile(string fileName, String fromPath, String toPath)
    {
      ...
      try
      {
        string path = Path.Combine(fromPath, fileName);
        fs = File.OpenRead(path);
        bytesRemaining = fs.Length;
        channel.openFileForWrite(fileName, toPath);
        while (true)
        {
 ...
 public class Comm
  ...
  public class TestPCommService
  {
    ...
    public static List<string> getClientFileList(String dir, String pattern)
    {
